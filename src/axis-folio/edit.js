...
export default function Edit( { attributes, setAttributes, clientId } ) {
    const { 
        uniqueId, items, columnsDesktop, columnsTablet, columnsMobile,
        borderRadius, hasShadow, shadowColor, showTags, cardBgColor,
        ...
        <div { ...useBlockProps() }>
            <InspectorControls>
                <PanelBody title={ __( 'Grid Layout', 'masionary' ) }>
                    <RangeControl 
                        label={ __( 'Columns (Desktop)', 'masionary' ) } 
                        value={ columnsDesktop } 
                        onChange={ ( val ) => setAttributes( { columnsDesktop: val } ) } 
                        min={ 1 } max={ 6 } 
                    />
                    <RangeControl 
                        label={ __( 'Columns (Tablet)', 'masionary' ) } 
                        value={ columnsTablet } 
                        onChange={ ( val ) => setAttributes( { columnsTablet: val } ) } 
                        min={ 1 } max={ 4 } 
                    />
                    <RangeControl 
                        label={ __( 'Columns (Mobile)', 'masionary' ) } 
                        value={ columnsMobile } 
            ...
            <InspectorControls group="styles">
                <PanelBody title={ __( 'Card Appearance', 'masionary' ) }>
                    ...
                    <RangeControl 
                        label={ __( 'Border Radius', 'masionary' ) } 
                        value={ borderRadius } 
                    ...
                    <ToggleControl 
                        label={ __( 'Drop Shadow', 'masionary' ) } 
                        checked={ hasShadow } 
                    ...
                    { hasShadow && (
                        <BaseControl label={ __( 'Shadow Color', 'masionary' ) }>
                            <ColorPicker
                                ...
                                onChange={ ( val ) => setAttributes( { shadowColor: val } ) }
                                enableAlpha
                                copyFormat="rgba"
                            />
                        </BaseControl>
                    ...
                    <ToggleControl 
                        label={ __( 'Enable Image Zoom', 'masionary' ) } 
                        checked={ hasZoom } 
                        onChange={ ( val ) => setAttributes( { hasZoom: val } ) } 
                    />

                    { hasZoom && (
                        <RangeControl 
                            label={ __( 'Zoom Intensity', 'masionary' ) } 
                            value={ zoomScale } 
                            onChange={ ( val ) => setAttributes( { zoomScale: val } ) } 
                ...
                { showTags && (
                    <PanelColorSettings
                        ...
                        colorSettings={ [
                            {
                                value: tagBgColor,
                                onChange: ( colorValue ) => setAttributes( { tagBgColor: colorValue } ),
                                label: __( 'Tag Background', 'masionary' ),
                            },
                            {
                                value: tagTextColor,
                                onChange: ( colorValue ) => setAttributes( { tagTextColor: colorValue } ),
                                label: __( 'Tag Text', 'masionary' ),
                            },
                        ] }
                    />
                ) }
                    ...
                    <div key={ index } className="item-card-wrapper" style={ { 
                        ...
                        <MediaUploadCheck>
                            <MediaUpload
                                ...
                                allowedTypes={ [ 'image' ] }
                                render={ ( { open } ) => (
                                    <div onClick={ open } style={ { background: '#f7f7f7', border: '1px dashed #ccc', padding: '20px', textAlign: 'center', cursor: 'pointer', marginBottom: '15px', overflow: 'hidden' } }>
